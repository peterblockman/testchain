...
import { EventCreateGroup } from "testchain-client-ts/cosmos.group.v1/types"
import { EventUpdateGroup } from "testchain-client-ts/cosmos.group.v1/types"
import { EventCreateGroupPolicy } from "testchain-client-ts/cosmos.group.v1/types"
import { EventUpdateGroupPolicy } from "testchain-client-ts/cosmos.group.v1/types"
import { EventSubmitProposal } from "testchain-client-ts/cosmos.group.v1/types"
import { EventWithdrawProposal } from "testchain-client-ts/cosmos.group.v1/types"
import { EventVote } from "testchain-client-ts/cosmos.group.v1/types"
import { EventExec } from "testchain-client-ts/cosmos.group.v1/types"
import { EventLeaveGroup } from "testchain-client-ts/cosmos.group.v1/types"
import { EventProposalPruned } from "testchain-client-ts/cosmos.group.v1/types"
import { EventTallyError } from "testchain-client-ts/cosmos.group.v1/types"
import { Member } from "testchain-client-ts/cosmos.group.v1/types"
import { MemberRequest } from "testchain-client-ts/cosmos.group.v1/types"
import { ThresholdDecisionPolicy } from "testchain-client-ts/cosmos.group.v1/types"
import { PercentageDecisionPolicy } from "testchain-client-ts/cosmos.group.v1/types"
import { DecisionPolicyWindows } from "testchain-client-ts/cosmos.group.v1/types"
import { GroupInfo } from "testchain-client-ts/cosmos.group.v1/types"
import { GroupMember } from "testchain-client-ts/cosmos.group.v1/types"
import { GroupPolicyInfo } from "testchain-client-ts/cosmos.group.v1/types"
import { Proposal } from "testchain-client-ts/cosmos.group.v1/types"
import { TallyResult } from "testchain-client-ts/cosmos.group.v1/types"
import { Vote } from "testchain-client-ts/cosmos.group.v1/types"
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						EventTallyError: getStructure(EventTallyError.fromPartial({})),
...
export default {
	...
	actions: {
		...
		async sendEventTallyError({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.CosmosGroupV1.tx.sendEventTallyError({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:EventTallyError:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:EventTallyError:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async EventTallyError({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.CosmosGroupV1.tx.eventTallyError({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:EventTallyError:Init Could not initialize signing client. Wallet is required.')
				} else{
					throw new Error('TxClient:EventTallyError:Create Could not create message: ' + e.message)
				}
			}
		},
